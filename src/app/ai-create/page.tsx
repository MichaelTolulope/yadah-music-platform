...
async function handleSaveToProject(lyric: AIResult & { audioUrl?: string }) {
  ...
  // 2. Determine a clean display title
  const finalTitle =
    form.title?.trim() ||
    form.prompt?.trim().slice(0, 60) ||
    `AI ${selectedTool.name} Draft`;

  // 3. Insert into the tracks table with the added audio assets
  const { error: insertError } = await supabase.from("tracks").insert({
    project_id: project.id,
    owner_id: userId,
    title: finalTitle,
    status: "draft",
    // Ensure this column name maps to your actual storage column (e.g., audio_url or path)
    audio_url: lyric.audioUrl || null, 
    metadata: {
      lyrics: lyric.lyrics,
      language: lyric.language,
      style: lyric.style,
      audioUrl: lyric.audioUrl || null, // ✅ Capture the Hugging Face generated file location

      // Preserving the precise user construction recipe:
      inputs: {
        core_prompt: form.prompt?.trim() || "",
        baseline_lyrics: form.lyrics?.trim() || "",
        custom_styles: form.styles?.trim() || "",
        extra_options: form.options?.trim() || ""
      }
    },
  });

  if (insertError) {
    console.error("Supabase Save Error Details:", insertError);
    setError(`Failed to save track: ${insertError.message}`);
  } else {
    // Optional UI polish: trigger temporary checkmark state or toast banner notice
    alert("Saved successfully to your active project!");
  }
}
